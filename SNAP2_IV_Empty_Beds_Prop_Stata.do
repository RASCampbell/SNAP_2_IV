* Stata Do file to run the IV analysis for POMS estimate with postoperative CCU admission, for the Sensitivity Analysis where the definition of an available CCU bed = Proportion of total CCU capacity that are empty beds at time of surgery
...
biprobit (outcome: POMS = i.icu_adm_dummy $xlist) (rxselect: icu_adm_dummy = $xlist PropEmptyTimeofSurgery), nolog  vce(cluster SiteCode)
...
replace parm_label = "Proportion of CCU beds that were empty at time of surgery" if parm == "PropEmptyTimeofSurgery"

list eq parm parm_label est min95 max95 p stars or_est or_min95 or_max95

* export excel sheet with results
export excel using "PropEmpty_IV_POMS_pars_mv_impute_norm_age_cat.xlsx", sheet("raw") sheetreplace firstrow(varlabels)
...
ivreg2 POMS	 ///
		$xlist ///
		(i.icu_adm_dummy = PropEmptyTimeofSurgery) ///
		, ///
...
* export excel sheet with results
export excel using "2SLS_PropEmpty_IV_POMS_pars_mv_impute_norm_age_cat.xlsx", sheet("raw") sheetreplace firstrow(varlabels)
