* Stata Do file to run the IV analysis for POMS estimate with postoperative CCU admission, for the Sensitivity Analysis where the definition of an available CCU bed = 0 Empty Beds vs 1 Empty Beds vs 2+ Empty Beds (Categorical)
...
biprobit (outcome: POMS = i.icu_adm_dummy $xlist) (rxselect: icu_adm_dummy = $xlist i.EmptyBedsTimeofSurgCat), nolog  vce(cluster SiteCode)
...
replace parm_label = "Empty beds at time of surgery = 0" if parm == "1b.EmptyBedsTimeofSurgCat"
replace parm_label = "Empty beds at time of surgery = 1" if parm == "2.EmptyBedsTimeofSurgCat"
replace parm_label = "Empty beds at time of surgery = 2 or more" if parm == "3.EmptyBedsTimeofSurgCat"

list eq parm parm_label est min95 max95 p stars or_est or_min95 or_max95

* export excel sheet with results
export excel using "tb_IV_POMS_pars_mv_impute_norm_age_cat.xlsx", sheet("raw") sheetreplace firstrow(varlabels)
...
ivreg2 POMS	 ///
		$xlist ///
		(i.icu_adm_dummy = i.EmptyBedsTimeofSurgCat) ///
		, ///
...
* export excel sheet with results
export excel using "2SLS_tb_IV_POMS_pars_mv_impute_norm_age_cat.xlsx", sheet("raw") sheetreplace firstrow(varlabels)
