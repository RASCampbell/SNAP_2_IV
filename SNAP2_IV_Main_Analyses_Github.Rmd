...
patients_labelled4 <- patients_labelled3 %>% 
  ...
  # 9-6-45-Com = Liver transplant, 9-6-46-Com = Liver and Kidney transplant, 9-6-47-Com = Bowel Transplant
    filter(!(specialty.factor == 'Transplant')) %>% # All other transplants
...
excluded_procedures_clean_patient_list <- patients_labelled3 %>% 
  ...
  (specialty.factor == 'Transplant')  
  ) %>% 
  dplyr::select(CaseId, SurgeryProcedure, SurgeryProcedureSeverity, MainGroup, Specialty.y)
...
odds_ratio_table_empty_random_effects_icu_adm <- empty_random_effects_icu_adm %>% broom.mixed::tidy(conf.int = TRUE, exp = TRUE) 
...
# variance = 1.04
my.var_empty_icu_adm <- 1.04 #AIC 12018, BIC 12033 (no. obs = 19386, sites 248)
...
my.var_random_effect_log_ccu_age_cat_impute_norm_critical_adverse_speciality_totalcap_include <- 1.38 
...
write.xlsx(odds_ratio_table_log_ccu_age_cat_impute_norm_critical_adverse_speciality_totalcap_include, file = 'odds_ratio_table_log_ccu_age_cat_impute_norm_critical_adverse_speciality_totalcap_include.xlsx', sheetName = "Sheet1", 
  col.names = TRUE, row.names = TRUE, append = FALSE)
```
...
iv_results_row <- c("Bivariate Probit", "icu_adm.factorTRUE", "", "", "", "", "", "", 0.91,	0.57,	1.45) # taken from bivariate_probit_results_pars_mv_impute_norm.rda
...
b <- c("icu_adm.factorTRUE", -0.010, 0.025, -0.38, 0.701, -0.059, 0.040, "Bivariate Probit IV analysis") # ATE results are obtained when running the Stata script above. Results are manually copied across from Stata. 
...
c <- c("icu_adm.factorTRUE", -0.010, 0.025, -0.38, 0.701, -0.059, 0.040, "Bivariate Probit IV analysis")
