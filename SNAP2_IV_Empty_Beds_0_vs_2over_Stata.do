* Stata Do file to run the IV analysis for POMS estimate with postoperative CCU admission, for the Sensitivity Analysis where the definition of an available CCU bed = 0 Empty Beds vs 2+ beds Empty Beds (Categorical but missing 1 Empty bed category). 
...
use empty_beds_0_vs_2_data, clear
...
replace parm_label = "Empty beds at time of surgery = 2 or more" if parm == "2.EmptyBedsTimeofSurgCat"

list eq parm parm_label est min95 max95 p stars or_est or_min95 or_max95

* export excel sheet with results
export excel using "empty_0_vs_2_IV_POMS_pars_mv_impute_norm_age_cat.xlsx", sheet("raw") sheetreplace firstrow(varlabels)
...
* export excel sheet with results
export excel using "2SLS_empty_0_vs_2_IV_POMS_pars_mv_impute_norm_age_cat.xlsx", sheet("raw") sheetreplace firstrow(varlabels)
