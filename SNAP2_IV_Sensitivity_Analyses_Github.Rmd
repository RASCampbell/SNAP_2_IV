...
This script was run using R Version 4.0.3 in RStudio Version 1.2.5033. It utilises results generated from Stata scripts run using Stata Release 17.
...
```{r}
# Complete case dataset for POMS
complete_case_data_poms <- patients_labelled6 %>% 
  dplyr::select(POMS, POMS.factor, icu_adm.factor,  S01Gender  , Age_scaled, age_p10  , weekend.factor  ,  S02OperativeUrgency  , S02PlannedProcSeverity  , asa  ,  PreopLOS.factor  ,  combined_DHx  , combined_PMHx  , diabetes.factor  ,  sysBP_scaled  , HR_scaled  ,  Anaesthetist_grade_combined.factor  , Surgeon_grade_combined.factor , abnormal_bloodtest.factor , S04BloodLoss , SiteCode, CCUCapacityTimeofSurgery )
...
complete_case_data_mort30 <- patients_labelled6 %>% 
  dplyr::select(mortality30.factor, icu_adm.factor,  S01Gender  , Age_scaled, age_p10  , weekend.factor  ,  S02OperativeUrgency  , S02PlannedProcSeverity  , asa  ,  PreopLOS.factor  ,  combined_DHx  , combined_PMHx  , diabetes.factor  ,  sysBP_scaled  , HR_scaled  ,  Anaesthetist_grade_combined.factor  , Surgeon_grade_combined.factor , abnormal_bloodtest.factor , S04BloodLoss , SiteCode,  CCUCapacityTimeofSurgery  )
...
complete_case_data_mort60 <- patients_labelled6 %>% 
  dplyr::select(mortality60.factor, icu_adm.factor,  S01Gender  , Age_scaled,age_p10  , weekend.factor  ,  S02OperativeUrgency  , S02PlannedProcSeverity  , asa  ,  PreopLOS.factor  ,  combined_DHx  , combined_PMHx  , diabetes.factor  ,  sysBP_scaled  , HR_scaled  ,  Anaesthetist_grade_combined.factor  , Surgeon_grade_combined.factor , abnormal_bloodtest.factor , S04BloodLoss , SiteCode,  CCUCapacityTimeofSurgery  )
...
# variance = 0.95
my.var_complete_case_empty_icu_adm <- 0.96
...
iv_results_row <- c("Bivariate Probit", "icu_adm.factorTRUE", "", "", "", "", "", "", 1.001,	0.61,	1.64)  # taken from bivariate_probit_results_pars_mv.rda
...
b <- c("icu_adm.factorTRUE", 0.0002, 0.031, 0.01, 0.996, -0.060, 0.061, "Bivariate Probit IV analysis") # ATE results are obtained when running the Stata script above. Results are manually copied across from Stata.
...
c <- c("icu_adm.factorTRUE", 0.0002, 0.031, 0.01, 0.996, -0.060, 0.061, "Bivariate Probit IV analysis")
...
iv_results_row <- c("Bivariate Probit", "icu_adm.factorTRUE", "", "", "", "", "", "", 0.88,	0.56,	1.40) # taken from IV results table above
...
b <- c("icu_adm.factorTRUE", -0.013, 0.024, -0.53, 0.593, -0.06, 0.03, "Bivariate Probit IV analysis")
...
c <- c("icu_adm.factorTRUE", -0.013, 0.024, -0.53, 0.593, -0.06, 0.03, "Bivariate Probit IV analysis")
...
SORT_levels_bivariate_probit_marginal_results[1,] <-  c("icu_adm.factorTRUE", -0.010, 0.025, -0.38, 0.701, -0.059, 0.040, "Overall")
SORT_levels_bivariate_probit_marginal_results[2,] <-  c("icu_adm.factorTRUE", 0.072, 0.077, 0.94, 0.345, -0.078, 0.22, "1+")
SORT_levels_bivariate_probit_marginal_results[3,] <-  c("icu_adm.factorTRUE", 0.023, 0.10, 0.23, 0.817, -0.174, 0.22, "2+")
SORT_levels_bivariate_probit_marginal_results[4,] <-  c("icu_adm.factorTRUE", -0.022, 0.11, -0.20, 0.838, -0.24, 0.19, "3+")
SORT_levels_bivariate_probit_marginal_results[5,] <-  c("icu_adm.factorTRUE", -0.11, 0.13, -0.82, 0.415, -0.37, 0.15, "4+")
SORT_levels_bivariate_probit_marginal_results[6,] <-  c("icu_adm.factorTRUE", -0.186, 0.127, -1.47, 0.142, -0.435, 0.062, "5+")
...
# Add ATE results into dataframe
variety_instruments_IV_AME_results[1,] <-  c("icu_adm.factorTRUE", -0.010, 0.025, -0.38, 0.701, -0.059, 0.040, "Available CCU beds (Empty + Discharge ready)")
variety_instruments_IV_AME_results[2,] <-  c("icu_adm.factorTRUE", -0.021, 0.025, -0.85, 0.397, -0.071, 0.028, "Empty CCU Beds")
variety_instruments_IV_AME_results[3,] <-  c("icu_adm.factorTRUE", -0.033, 0.024, -1.38, 0.168, -0.081, 0.014, "Empty CCU Beds: Categories (0 vs 2+ beds)")
variety_instruments_IV_AME_results[4,] <-  c("icu_adm.factorTRUE", -0.030, 0.024, -1.24, 0.214, -0.078, 0.017, "Empty CCU Beds: Categories (0,1,2+ beds)")
variety_instruments_IV_AME_results[5,] <-  c("icu_adm.factorTRUE", -0.029, 0.026, -1.13, 0.258, -0.080, 0.021, "Proportion of CCU beds that are empty")
...
## Random effects
DescTools::Cstat(x = predict(mort30_impute_norm_random_effect, method="response"),                                                                     resp = model.response(model.frame(mort30_impute_norm_random_effect))) # 0.93
...
```{r}
## Model A
DescTools::Cstat(x = predict(planned_only_empty_random_effects_icu_adm, method="response"),                                                                     resp = model.response(model.frame(planned_only_empty_random_effects_icu_adm))) # 0.77
...
## 3+
DescTools::Cstat(x = predict(log_poms_age_cat_impute_norm_pars_sort3over_clean_random_effect, method="response"),                                                                     resp = model.response(model.frame(log_poms_age_cat_impute_norm_pars_sort3over_clean_random_effect))) # 0.73

## 4+
DescTools::Cstat(x = predict(log_poms_age_cat_impute_norm_pars_sort4over_clean_random_effect, method="response"),                                                                     resp = model.response(model.frame(log_poms_age_cat_impute_norm_pars_sort4over_clean_random_effect))) # 0.70

## 5+
DescTools::Cstat(x = predict(log_poms_age_cat_impute_norm_pars_sort5over_clean_random_effect, method="response"),                                                                     resp = model.response(model.frame(log_poms_age_cat_impute_norm_pars_sort5over_clean_random_effect))) # 0.69
```
